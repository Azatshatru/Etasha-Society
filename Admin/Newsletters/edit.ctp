...
$uploaded = (($newsletter->file_name && file_exists($imageRoot.$newsletter->file_name))?$this->Html->link($this->Html->tag('i', '', ['class' => 'fa fa-download fa-lg']), $this->Url->build('/', true).$imageDir.$newsletter->file_name, ['escape' => false, 'class' => 'input-group-addon btn green', 'target' => '_blank']):'');
?>
<?= $this->Flash->render(); ?>
    ...
    <div class="portlet-title">
        ...
        <div class="actions btn-set">
            ...
            <?= $this->Form->button($this->Html->tag('i', '', ['class' => 'fa fa-check']).
                __(' Save Newsletter'), ['escape' => false, 'class' => 'btn btn-success']); ?>
            <?= $this->Form->postLink($this->Html->tag('i', '', ['class' => 'fa fa-trash']).
                __(' Delete'), ['action' => 'delete', $newsletter->id]+$pageNo, ['escape' => false, 'block' => true, 'class'=>'btn btn-danger', 'confirm' => __('Are you sure you want to delete download #{0}?', $newsletter->id)]); ?>
        </div>
    </div>
    <div class="portlet-body">
        <div class="tabbable-bordered">
            <div class="row">
				...
				<div class="col-xs-12 col-sm-6">
                   <?= $this->Form->control('newsletter_category_id', ['label' => ['text' => __('Newsletter Category'), 'class' => 'control-label'], 'class' => 'form-control', 'options' => $newsletterCategory, 'empty' => __('Select Newsletter Category'), 'default' => 1, 'required' => true, 'templates' => 'form_bootstrap']); ?>
                </div>
            </div>
