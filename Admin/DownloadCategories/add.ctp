...
/* $(document).on('change', '#parent-id', function() {
    var selectedInstitutes = $.map($('#institutes-ids :selected') ,function(option) {
        return option.value;
    });
    
    $.ajax({
        method: 'POST',
        url: '".$this->Url->build(['action' => 'fillInstitutes'])."',
        dataType: 'html',
        data:{parent_id:$('#parent-id').val()},
        cache: false,
        beforeSend: function() {
            $('#ajax-indicator').fadeIn();
            $('#institutes-ids').html('').selectpicker('refresh');
        }
    }).done(function(data) {
        $('#institutes-ids').html(data).val(selectedInstitutes).selectpicker('refresh');
    }).always(function() {
