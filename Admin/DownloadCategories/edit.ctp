...
<?= $this->Form->create($downloadCategory, ['type' => 'file']); ?>
<div class="portlet">
    <div class="portlet-title">
        ...
        <div class="actions btn-set">
            ...
            <?= $this->Form->button($this->Html->tag('i', '', ['class' => 'fa fa-check']).
                __(' Save Download Category'), ['escape' => false, 'class' => 'btn btn-success']); ?>
            <?= $this->Form->postLink($this->Html->tag('i', '', ['class' => 'fa fa-trash']).
                __(' Delete'), ['action' => 'delete', $downloadCategory->id]+$pageNo, ['escape' => false, 'block' => true, 'class'=>'btn btn-danger', 'confirm' => __('Are you sure you want to delete download category #{0}?', $downloadCategory->id)]); ?>
        </div>
    </div>
        ...
        <div class="tabbable-bordered">
            ...
            <!--<div class="row">
                <div class="col-xs-12 col-sm-6">
					...
					$fdate = date('d-m-Y',strtotime($downloadCategory->from_date));
					$tdate = date('d-m-Y',strtotime($downloadCategory->to_date));
					?>
                    <?= $this->Form->control('from_date', ['label' => ['text' => __('Published From'), 'class' => 'control-label'], 'class' => 'form-control', 'type' => 'text','value'=>@$fdate, 'templates' => 'form2_bootstrap', 'templateVars' => ['picker' => $picker]]); ?>
                </div>
                <div class="col-xs-12 col-sm-6">
                    <?= $this->Form->control('to_date', ['label' => ['text' => __('Published To'), 'class' => 'control-label'], 'class' => 'form-control', 'type' => 'text', 'templates' => 'form2_bootstrap','value'=>@$tdate, 'templateVars' => ['picker' => $picker]]); ?>
                </div>
            </div>
            <div class="row">
                <div class="col-xs-12">
                <label>
					<h3>H3 title</h3>
					<h4>H4 title</h4>
					<h4>.liststyle1 = List with Bullets</h4>
					<h4>.imgborder = For image Border</h4>
				</label>
                </div>
            </div>
            <div class="row">
