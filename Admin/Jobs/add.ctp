...
$form3_bootstrap = [
    ...
    'input' => '<div class="input-group date date-picker" data-date-format="dd-mm-yyyy" data-date-start-date="+0d"><input type="{{type}}" name="{{name}}"{{attrs}}/>{{picker}}</div>',
];
?>
<?= $this->Flash->render(); ?>
        ...
        <div class="tabbable-bordered">
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-6">
                    ...
                    <?= $this->Form->control('from_date', ['label' => ['text' => __('Published From'), 'class' => 'control-label'], 'class' => 'form-control', 'type' => 'text', 'templates' => $form3_bootstrap, 'default' => $this->Time->format('', 'dd-MM-yyyy'), 'templateVars' => ['picker' => $picker]]); ?>
                </div>
                <div class="col-xs-12 col-sm-6">
                    <?= $this->Form->control('to_date', ['label' => ['text' => __('Published To'), 'class' => 'control-label'], 'class' => 'form-control', 'type' => 'text', 'templates' => $form3_bootstrap, 'default' => $this->Time->format('+1 Year', 'dd-MM-yyyy'), 'templateVars' => ['picker' => $picker]]); ?>
                </div>
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-6 in-row">
                    <?= $this->Form->control('top_on_list', ['label' => ['text' => __('Move the job at the top of the list.'), 'class' => 'mt-checkbox mt-checkbox-outline'], 'class' => 'form-control', 'type' => 'checkbox', 'default' => 1, 'templates' => 'form_bootstrap']) ?>
                </div>
            </div>
			<div class="row">
				<div class="col-xs-12">
