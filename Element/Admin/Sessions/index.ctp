    ...
    <table class="table table-striped table-bordered table-hover dataTable">
        <thead>
            <tr role="row" class="heading sortable-control">
                <th><?= $this->Paginator->sort('Sessions.name', __('Name')); ?></th>
                <th><?= $this->Paginator->sort('Sessions.created', __('Date Added')); ?></th>
                <th><?= $this->Paginator->sort('Sessions.lft', __('Order')); ?></th>
                <th><?= $this->Paginator->sort('Sessions.status', __('Status')); ?></th>
                <th><?= __('Actions'); ?></th>
            </tr>
        ...
        <tbody>
            <?php if($this->Paginator->counter('{{count}}')): ?>
                ...
                <tr>
                    ...
                    <td><?= $this->Time->format($session->created, 'dd MMM yyyy hh:mm a') ?></td>
                    <td class="text-center"><?php
                    $categoryTree = array_keys($treeList->toArray(), $session->parent_id);
                    if($this->Paginator->sortKey() == 'Sessions.lft')
                    {
                        $currentKeyId = array_search($session->id, $categoryTree);
                        ...
                        if($nextNode)
                        {
                            echo $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-chevron-circle-down']), 
                                ['action' => 'move', 'down', $session->id]+$pageNo, ['class'=>'font-grey-mint', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to move course #{0} down?', $session->id)]);
                        }
                        else
                        {
                        ...
                        if($previousNode)
                        {
                            echo ' '.$this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-chevron-circle-up']), 
                                ['action' => 'move', 'up', $session->id]+$pageNo, ['class'=>'font-grey-mint', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to move course #{0} up?', $session->id)]);
                        }
                        else
                        {
                            echo ' '.$this->Html->link($this->Html->tag('i', '', ['class' => 'fa fa-circle']), 
                    ...
                    {
                        echo $this->Html->link('#'.(array_search($session->id, $categoryTree) + 1), 'javascript:void(0);', ['class' => 'font-blue-steel', 'escape' => false]);
                    }
                    ?></td>
                    <td class="text-center"><?php
                    if($session->status):
                        echo $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-check']), 
                            ['action' => 'statusChange', 'inactive', $session->id]+$pageNo, ['class' => 'font-blue-steel', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to inactive course #{0}?', $session->id)]);
                    else:
                        echo $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-times']), 
                            ['action' => 'statusChange', 'active', $session->id]+$pageNo, ['class' => 'font-red', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to active course #{0}?', $session->id)]);
                    endif;
                    ?></td>
                    <td class="text-center">
                        <?= $this->Html->link($this->Html->tag('i', '', ['class' => 'fa fa-edit']), 
                            ['action' => 'edit', $session->id]+$pageNo, ['class'=>'font-blue-steel', 'escape' => false]); ?>
                        <?= $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-trash-o']), 
                            ['action' => 'delete', $session->id]+$pageNo, ['class'=>'font-red', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to delete course #{0}?', $session->id)]); ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            <?php else: ?>
