    ...
    <table class="table table-striped table-bordered table-hover dataTable">
        <thead>
            <tr role="row" class="heading sortable-control">
                <th>
                    <?= $this->Paginator->sort('NewsletterCategories.name', __('Name')); ?>
                </th>
                <?php if($imgMaximumLevel > 1): ?>
                <th><?= $this->Paginator->sort('ParentNewsletterCategories.name', __('Parent Category')); ?></th>
                <?php endif; ?>
                <th><?= $this->Paginator->sort('NewsletterCategories.created', __('Date Added')); ?></th>
                <th><?= $this->Paginator->sort('NewsletterCategories.lft', __('Order')); ?></th>
                <th><?= $this->Paginator->sort('NewsletterCategories.status', __('Status')); ?></th>
                <th><?= __('Actions'); ?></th>
            </tr>
        ...
        <tbody>
            <?php if($this->Paginator->counter('{{count}}')): ?>
                <?php foreach($newsletterCategories as $newsletterCategorie): ?>
                <tr>
                   <td class="multi">
                        <?= array_key_exists($newsletterCategorie->id, $treeList->toArray())?h($treeList->toArray()[$newsletterCategorie->id]):h($newsletterCategorie->name); ?>
                    </td>
                    <?php if($imgMaximumLevel > 1): ?>
                    <td><?= $newsletterCategorie->has('parent_newsletter_category')?array_key_exists($newsletterCategorie->parent_newsletter_category->id, $treeList->toArray())?h($treeList->toArray()[$newsletterCategorie->parent_newsletter_category->id]):h($newsletterCategorie->parent_newsletter_category->name):'' ?></td>
                    <?php endif; ?>
                  
                    <td><?= $this->Time->format($newsletterCategorie->created, 'dd MMM yyyy hh:mm a') ?></td>
                    <td class="text-center"><?php
                    $categoryTree = array_keys($treeList2->toArray(), $newsletterCategorie->parent_id);
                    if($this->Paginator->sortKey() == 'NewsletterCategories.lft')
                    {
                        $currentKeyId = array_search($newsletterCategorie->id, $categoryTree);
                        ...
                        if($nextNode)
                        {
                            echo $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-chevron-circle-down']), 
                                ['action' => 'move', 'down', $newsletterCategorie->id]+$pageNo, ['class'=>'font-grey-mint', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to move image category #{0} down?', $newsletterCategorie->id)]);
                        }
                        else
                        {
                        ...
                        if($previousNode)
                        {
                            echo ' '.$this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-chevron-circle-up']), 
                                ['action' => 'move', 'up', $newsletterCategorie->id]+$pageNo, ['class'=>'font-grey-mint', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to move image category #{0} up?', $newsletterCategorie->id)]);
                        }
                        else
                        {
                            echo ' '.$this->Html->link($this->Html->tag('i', '', ['class' => 'fa fa-circle']), 
                    ...
                    {
                        echo $this->Html->link('#'.(array_search($newsletterCategorie->id, $categoryTree) + 1), 'javascript:void(0);', ['class' => 'font-blue-steel', 'escape' => false]);
                    }
                    ?></td>
                    <td class="text-center"><?php
                    if($newsletterCategorie->status):
                        echo $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-check']), 
                            ['action' => 'statusChange', 'inactive', $newsletterCategorie->id]+$pageNo, ['class' => 'font-blue-steel', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to inactive newsletter category #{0}?', $newsletterCategorie->id)]);
                    else:
                        echo $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-times']), 
                            ['action' => 'statusChange', 'active', $newsletterCategorie->id]+$pageNo, ['class' => 'font-red', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to active newsletter category #{0}?', $newsletterCategorie->id)]);
                    endif;
                    ?></td>
                    <td class="text-center">
                       
                        <?= $this->Html->link($this->Html->tag('i', '', ['class' => 'fa fa-edit']), 
                            ['action' => 'edit', $newsletterCategorie->id]+$pageNo, ['class'=>'font-blue-steel', 'escape' => false]); ?>
                       
                        <?= $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-trash-o']), 
                            ['action' => 'delete', $newsletterCategorie->id]+$pageNo, ['class'=>'font-red', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to delete newsletter category #{0}?', $newsletterCategorie->id)]); ?>
