    ...
    <table class="table table-striped table-bordered table-hover dataTable">
        <thead>
            <tr role="row" class="heading sortable-control">
                <th><?= $this->Paginator->sort('News.name', __('Title')); ?></th>
                <th><?= $this->Paginator->sort('News.from_date', __('Published From')); ?></th>
                <th><?= $this->Paginator->sort('News.to_date', __('Published To')); ?></th>
                <th><?= $this->Paginator->sort('News.created', __('Date Added')); ?></th>
                <th><?= $this->Paginator->sort('News.lft', __('Order')); ?></th>
                <th><?= $this->Paginator->sort('News.status', __('Status')); ?></th>
                <th><?= __('Actions'); ?></th>
            </tr>
        ...
        <tbody>
            <?php if($this->Paginator->counter('{{count}}')): ?>
                <?php foreach($news as $newsLine): ?>
                <tr>
                    <td><?= h($newsLine->name); ?></td>
                    <td><?= $newsLine->from_date?$this->Time->format($newsLine->from_date, 'dd MMM yyyy'):''; ?></td>
                    <td><?= $newsLine->to_date?$this->Time->format($newsLine->to_date, 'dd MMM yyyy'):''; ?></td>
                    <td><?= $this->Time->format($newsLine->created, 'dd MMM yyyy hh:mm a') ?></td>
                    <td class="text-center"><?php
                    $categoryTree = array_keys($treeList->toArray(), $newsLine->news_category_id);
                    if($this->Paginator->sortKey() == 'News.lft')
                    {
                        $currentKeyId = array_search($newsLine->id, $categoryTree);
                        ...
                        if($nextNode)
                        {
                            echo $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-chevron-circle-down']), 
                                ['action' => 'move', 'down', $newsLine->id]+$pageNo, ['class'=>'font-grey-mint', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to move news #{0} down?', $newsLine->id)]);
                        }
                        else
                        {
                        ...
                        if($previousNode)
                        {
                            echo ' '.$this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-chevron-circle-up']), 
                                ['action' => 'move', 'up', $newsLine->id]+$pageNo, ['class'=>'font-grey-mint', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to move news #{0} up?', $newsLine->id)]);
                        }
                        else
                        {
                            echo ' '.$this->Html->link($this->Html->tag('i', '', ['class' => 'fa fa-circle']), 
                    ...
                    {
                        echo $this->Html->link('#'.(array_search($newsLine->id, $categoryTree) + 1), 'javascript:void(0);', ['class' => 'font-blue-steel', 'escape' => false]);
                    }
                    ?></td>
                    <td class="text-center"><?php
                    if($newsLine->status):
                        echo $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-check']), 
                            ['action' => 'statusChange', 'inactive', $newsLine->id]+$pageNo, ['class' => 'font-blue-steel', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to inactive news #{0}?', $newsLine->id)]);
                    else:
                        echo $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-times']), 
                            ['action' => 'statusChange', 'active', $newsLine->id]+$pageNo, ['class' => 'font-red', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to active news #{0}?', $newsLine->id)]);
                    endif;
                    ?></td>
                    <td class="text-center">
                        
                        <?= $this->Html->link($this->Html->tag('i', '', ['class' => 'fa fa-edit']), 
                            ['action' => 'edit', $newsLine->id]+$pageNo, ['class'=>'font-blue-steel', 'escape' => false]); ?>
                        
                        <?= $this->FormAjax->postLink($this->Html->tag('i', '', ['class' => 'fa fa-trash-o']), 
                            ['action' => 'delete', $newsLine->id]+$pageNo, ['class'=>'font-red', 'escape' => false, 'block' => true, 'confirm' => __('Are you sure you want to delete news #{0}?', $newsLine->id)]); ?>
